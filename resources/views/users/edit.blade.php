<x-app-layout>
    ...
    <form action="{{ route('users.update', $user) }}" method="POST" class="max-w-lg space-y-4 rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
        ...
        <div class="flex items-center gap-2">
            <input id="is_admin" name="is_admin" type="checkbox" value="1" class="rounded border-neutral-300 text-primary shadow-sm focus:ring-primary" @checked(old('is_admin', $user->is_admin))>
            <x-input-label for="is_admin" value="Administrateur" class="!mb-0" />
        </div>
