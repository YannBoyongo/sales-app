...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased">
        <div
            class="min-h-screen flex bg-slate-100"
            x-data="{
                mobileOpen: false,
                desktopExpanded: localStorage.getItem('app.sidebarDesktopOpen') !== '0',
                isDesktop() {
                    return window.matchMedia('(min-width: 1024px)').matches;
                },
                init() {
                    this.$watch('desktopExpanded', (value) => {
                        localStorage.setItem('app.sidebarDesktopOpen', value ? '1' : '0');
                    });
                },
                toggleSidebar() {
                    if (this.isDesktop()) {
        ...
        >
            ...
            <aside
                class="app-sidebar fixed inset-y-0 left-0 z-50 flex w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar text-white shadow-card transition-all duration-200 ease-out -translate-x-full lg:static lg:translate-x-0"
                :class="{
                    'translate-x-0': mobileOpen,
                    'app-sidebar--collapsed': ! desktopExpanded,
                }"
            >
                <div class="app-sidebar-header flex h-16 items-center justify-between border-b border-sidebar-border px-4">
                    <a href="{{ route('dashboard') }}" class="app-sidebar-brand min-w-0">
                        <p class="app-sidebar-brand-text truncate text-lg font-bold tracking-tight text-white">{{ $appSetting?->shopname ?? config('app.name') }}</p>
                        @if ($appSetting?->phone)
                            <p class="app-sidebar-brand-text truncate text-xs text-sidebar-muted">{{ $appSetting->phone }}</p>
                        @endif
                    </a>
                    <button
                        type="button"
                        ...
                        @click="toggleSidebar()"
                        :aria-label="isDesktop() ? (desktopExpanded ? 'Réduire le menu' : 'Agrandir le menu') : 'Fermer le menu'"
                        :title="isDesktop() ? (desktopExpanded ? 'Réduire le menu' : 'Agrandir le menu') : 'Fermer le menu'"
                    >
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path x-show="desktopExpanded || ! isDesktop()" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7" />
                            <path x-cloak x-show="isDesktop() && ! desktopExpanded" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7" />
                        </svg>
                    </button>
                </div>

                <nav class="flex-1 space-y-0.5 overflow-y-auto px-3 py-4 text-sm font-medium" @click="if (! isDesktop() && $event.target.closest('a')) closeMobile()">
                    <a href="{{ route('dashboard') }}" class="app-nav-link {{ request()->routeIs('dashboard') ? 'app-nav-link-active' : '' }}" title="Tableau de bord">
                    ...
                    @if (Auth::user()->canManageApplication())
                        ...
                        <a href="{{ route('branches.index') }}" class="app-nav-link {{ request()->routeIs('branches.*') ? 'app-nav-link-active' : '' }}" title="Branches">
                        ...
                        <a href="{{ route('departments.index') }}" class="app-nav-link {{ request()->routeIs('departments.*') ? 'app-nav-link-active' : '' }}" title="Départements">
                    ...
                    @endif
                    <a href="{{ route('products.index') }}" class="app-nav-link {{ request()->routeIs('products.*') ? 'app-nav-link-active' : '' }}" title="Produits">
                    ...
                    @endif
                    <a href="{{ route('stock-movements.index') }}" class="app-nav-link {{ request()->routeIs('stock-movements.*') ? 'app-nav-link-active' : '' }}" title="Mouvements de stock">
                    ...
                    <a href="{{ route('requisitions.index') }}" class="app-nav-link {{ request()->routeIs('requisitions.*') ? 'app-nav-link-active' : '' }}" title="Réquisition">
                    ...
                    <a href="{{ route('purchase-orders.index') }}" class="app-nav-link {{ request()->routeIs('purchase-orders.*') ? 'app-nav-link-active' : '' }}" title="Bons de commande">
                    ...
                    @if (Auth::user()->canAccessPosSales())
                        <a href="{{ route('sales.entry') }}" class="app-nav-link {{ request()->routeIs(['sales.entry', 'sales.choose-terminal', 'sales.choose-department', 'sales.create', 'sales.store', 'pos-terminal.workspace', 'pos-terminal.shifts.open', 'pos-terminal.shifts.close-review', 'pos-terminal.shifts.close']) ? 'app-nav-link-active' : '' }}" title="Terminal POS">
                    ...
                    @if (Auth::user()->canAccessCashDeskFinanceFeatures())
                        ...
                        @if (Auth::user()->canAccessAccounting())
                            <a href="{{ route('accounting.index') }}" class="app-nav-link {{ request()->routeIs('accounting.*') ? 'app-nav-link-active' : '' }}" title="Comptabilité">
                        ...
                        @if (Auth::user()->canAccessAccounting())
                            <a href="{{ route('chart-of-accounts.index') }}" class="app-nav-link {{ request()->routeIs('chart-of-accounts.*') ? 'app-nav-link-active' : '' }}" title="Plan comptable">
                    ...
                    @if (Auth::user()->canManageApplication())
                        ...
                        <a href="{{ route('parametre.edit') }}" class="app-nav-link {{ request()->routeIs('parametre.*') ? 'app-nav-link-active' : '' }}" title="Paramètre">
                        ...
                        <a href="{{ route('users.index') }}" class="app-nav-link {{ request()->routeIs('users.*') ? 'app-nav-link-active' : '' }}" title="Gestion des utilisateurs">
                ...
                <div class="app-sidebar-footer border-t border-sidebar-border p-3 text-sm">
                    <div class="app-sidebar-user flex items-center gap-3 rounded-lg bg-white/10 px-3 py-2.5" title="{{ Auth::user()->name }}">
                        <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/20 text-sm font-bold text-white">
                            {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                        </span>
                        <div class="app-sidebar-user-meta min-w-0 flex-1">
                    ...
                    <div class="mt-2 flex flex-col gap-0.5">
                        <a href="{{ route('profile.edit') }}" class="app-nav-link" title="Profil">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="app-nav-link w-full text-left" title="Déconnexion">
            ...
            <div class="flex min-w-0 flex-1 flex-col">
                <header class="sticky top-0 z-30 flex h-16 shrink-0 items-center justify-between border-b border-slate-200/80 bg-white/90 px-4 shadow-sm backdrop-blur-md lg:px-8">
                    <div class="flex min-w-0 items-center gap-3">
                        <button
                            type="button"
                            class="rounded-lg border border-slate-200 p-2 text-slate-700 transition hover:bg-slate-50"
                            @click="toggleSidebar()"
                            :aria-label="isDesktop() ? (desktopExpanded ? 'Réduire le menu' : 'Agrandir le menu') : (mobileOpen ? 'Fermer le menu' : 'Ouvrir le menu')"
                            :title="isDesktop() ? (desktopExpanded ? 'Réduire le menu' : 'Agrandir le menu') : (mobileOpen ? 'Fermer le menu' : 'Ouvrir le menu')"
                        >
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
