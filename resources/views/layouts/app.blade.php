    ...
    <body class="font-sans antialiased bg-neutral-50 text-neutral-900">
        <div class="min-h-screen flex" x-data="{ sidebarOpen: false }">
            ...
            <aside
                ...
                class="fixed lg:static inset-y-0 left-0 z-50 w-64 shrink-0 bg-neutral-950 text-white flex flex-col border-r border-neutral-800 transition-transform duration-200 ease-out lg:transition-none"
            >
                ...
                <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1 text-sm font-medium" @click="if (window.matchMedia('(max-width: 1023px)').matches && $event.target.closest('a')) sidebarOpen = false">
                    ...
                    @if (Auth::user()->canAccessPosSales())
                        <p class="pt-4 pb-1 px-3 text-xs uppercase tracking-wider text-neutral-500">Caisse</p>
                        <a href="{{ route('sales.entry') }}" class="flex items-center gap-3 rounded-md px-3 py-2 {{ request()->routeIs(['sales.entry', 'sales.choose-terminal', 'sales.choose-department', 'sales.create', 'sales.store', 'pos-terminal.workspace', 'pos-terminal.shifts.open', 'pos-terminal.shifts.close-review', 'pos-terminal.shifts.close']) ? 'bg-primary text-white' : 'text-neutral-300 hover:bg-neutral-800 hover:text-white' }}">
