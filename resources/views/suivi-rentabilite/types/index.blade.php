<x-app-layout>
    ...
    <x-caisse-flow max-width="max-w-4xl" :with-card="false">
        ...
        <form action="{{ route('suivi-rentabilite.types.store') }}" method="POST" class="mb-6 flex flex-col gap-3 rounded-xl border border-neutral-200 bg-white p-4 shadow-sm sm:flex-row sm:items-end">
            @csrf
            <div class="flex-1">
                        ...
                        <tr class="transition-colors hover:bg-neutral-50/80">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex flex-wrap items-center justify-end gap-2">
                                    <a href="{{ route('suivi-rentabilite.types.edit', $type) }}" class="inline-flex items-center rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-xs font-semibold text-neutral-700 shadow-sm hover:bg-neutral-50">
                                        Modifier
                                    </a>
                                    <form
                                        action="{{ route('suivi-rentabilite.types.destroy', $type) }}"
                                        method="POST"
                                        class="inline"
