<x-app-layout>
    <x-slot name="header">Modifier l’emplacement</x-slot>

    <x-page-header title="Modifier l’emplacement" />

    <form action="{{ route('locations.update', $location) }}" method="POST" class="max-w-lg space-y-4 rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
        @csrf
        @method('PATCH')
        <div>
            ...
            <select id="branch_id" name="branch_id" class="mt-1 block w-full rounded-md border-neutral-300 shadow-sm focus:border-primary focus:ring-primary" required>
                @foreach ($branches as $b)
                    <option value="{{ $b->id }}" @selected(old('branch_id', $location->branch_id) == $b->id)>{{ $b->name }}</option>
                @endforeach
            </select>
