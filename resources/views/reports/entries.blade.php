<x-app-layout>
    <x-slot name="header">Liste des entrées</x-slot>
        ...
        <x-slot name="header">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <h1 class="app-page-title">Liste des entrées</h1>
                    <p class="app-page-desc max-w-3xl">
                        Entrées de stock par produit (réceptions, saisies manuelles), regroupées sur la période. Une ligne par produit.
                    </p>
                </div>
                @include('reports.partials.print-button', ['route' => 'reports.entries.pdf'])
            </div>
        </x-slot>

        <form method="GET" action="{{ route('reports.entries') }}" class="app-filter-bar mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div>
            ...
            <div class="flex items-end gap-2 sm:col-span-2">
                ...
                <a href="{{ route('reports.entries') }}" class="app-btn-secondary">Réinitialiser</a>
            </div>
        </form>
                ...
                <tbody class="divide-y divide-neutral-100 bg-white">
                    @forelse ($rows as $row)
                        <tr class="transition-colors hover:bg-neutral-50/80">
                            ...
                            <td class="px-4 py-3 text-neutral-700">{{ $row->locations ?: '-' }}</td>
                            <td class="px-4 py-3 text-right tabular-nums font-semibold text-neutral-900">{{ number_format((int) $row->total_quantity, 0, ',', ' ') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-14 text-center text-neutral-500">Aucune entrée enregistrée pour cette période.</td>
                        </tr>
                    @endforelse
                </tbody>
                @if ($rows->isNotEmpty())
                    <tfoot class="border-t-2 border-neutral-200 bg-neutral-50/90">
                        <tr>
                            <td colspan="4" class="px-4 py-3 font-semibold text-neutral-900">Total</td>
