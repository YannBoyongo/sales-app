<x-app-layout>
    <x-slot name="header">Modifier {{ $purchaseOrder->reference }}</x-slot>

    <x-caisse-flow
        max-width="max-w-4xl"
        eyebrow="Achats"
        title="Modifier le bon de commande"
        :description="'Tant qu’aucune réception n’a commencé, vous pouvez ajuster la référence, l’emplacement et les lignes.'"
        :with-card="true"
    >
        <form
            ...
            class="space-y-6"
            x-data="{
                rows: @js(old('products', $purchaseOrder->items->map(fn ($item) => ['product_id' => (string) $item->product_id, 'quantity_ordered' => (int) $item->quantity_ordered])->values()->all()))
            }"
        >
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <x-text-input id="reference" name="reference" type="text" class="mt-1 block w-full font-mono" :value="old('reference', $purchaseOrder->reference)" required maxlength="100" autocomplete="off" />
                <p class="mt-1 text-xs text-neutral-500">Référence unique du bon de commande.</p>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <select id="location_id" name="location_id" class="mt-1 block w-full rounded-md border-neutral-300 shadow-sm focus:border-primary focus:ring-primary" required>
                        ...
                        @foreach ($locations as $loc)
                            <option value="{{ $loc->id }}" @selected(old('location_id', $purchaseOrder->location_id) == $loc->id)>{{ $loc->name }} ({{ $loc->branch->name }})</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="supplier" name="supplier" type="text" class="mt-1 block w-full" :value="old('supplier', $purchaseOrder->supplier)" />
            ...
            <div class="flex flex-wrap gap-3 border-t border-neutral-100 pt-6">
                <x-primary-button>Mettre à jour le Bon de commande</x-primary-button>
                <a href="{{ route('purchase-orders.show', $purchaseOrder) }}" class="app-btn-secondary">Annuler</a>
            </div>
        </form>
    </x-caisse-flow>
</x-app-layout>
