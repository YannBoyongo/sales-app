<x-app-layout>
    ...
    <x-caisse-flow max-width="max-w-lg" :with-card="false">
        ...
        <form action="{{ route('suivi-cout.centres.update', $costCenter) }}" method="POST" class="space-y-4 rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
            @csrf
            @method('PUT')
            <div>
            ...
            <div class="flex flex-wrap gap-2 border-t border-neutral-100 pt-4">
                ...
                <a href="{{ route('suivi-cout.centres.index') }}" class="app-btn-secondary">Annuler</a>
            </div>
        </form>
    </x-caisse-flow>
</x-app-layout>
