            ...
            @else
                <ul class="max-h-48 space-y-2 overflow-y-auto">
                    @foreach ($stockManagerCandidates as $candidate)
                        @php
                            $defaultIds = $location->stockManagers->pluck('id')->map(fn ($id) => (int) $id)->all();
                            $selected = collect(old('stock_manager_ids', $defaultIds))->map(fn ($id) => (int) $id)->contains((int) $candidate->id);
                        @endphp
