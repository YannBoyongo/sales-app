<x-app-layout>
    <x-slot name="header">Nouvelle branche</x-slot>

    <x-page-header title="Nouvelle branche" />

    <form action="{{ route('branches.store') }}" method="POST" class="max-w-lg space-y-4 rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
        @csrf
        <div>
            ...
            <div class="rounded-lg border border-neutral-200 bg-neutral-50 p-4">
                <label class="flex cursor-pointer items-start gap-3">
                    <input
                        ...
                        @checked(old('can_sell_on_credit'))
                    />
                    <span>
            ...
            <div class="rounded-lg border border-neutral-200 bg-neutral-50 p-4">
                <label class="flex cursor-pointer items-start gap-3">
                    <input
                        ...
                        value="1"
                        class="mt-1 rounded border-neutral-300 text-primary focus:ring-primary"
                        @checked(old('can_apply_discount'))
                    />
                    <span>
